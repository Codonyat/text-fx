...
JSX / standalone `.html` / PNG / share-link). 196 effects across 13 categories; no backend.
...
- `lib/effects/<category>/<id>.ts` — one effect each (default export). `registry.ts` is **hand-maintained** (imports all + `EFFECTS` array → `MANIFEST`); add the import + array entry when adding/removing an effect file. `taxonomy.ts` = the 13 categories. `catalog.json` = the 322-effect research catalog (implementation backlog, not bundled; tiers partition all entries — new ideas go in `tiers.v2Candidates`). `gap-report.md` = the 2026-07 coverage/gap analysis (built-vs-catalog match map + ranked open gaps).
- `components/` — `Studio.tsx` (orchestrator/state) · two-layer `Stage.tsx` (single-element effects edit in place; per-letter AND composite-markup effects — any root with element children: letter spans, ticker tracks, stacked copies, inline SVG — use an editable ghost layer over a preview layer; never mutate the editable node into spans; the two render branches carry distinct `key`s + the text-sync effect deps on the composite flag, so a branch switch remounts the editable instead of reusing the div and orphaning its imperatively-set text node) · `StyleHost.tsx` (owns all `<style>` tags) · `EffectPreview.tsx` (server-side SSR preview for SEO pages) · Header/ActionBar/AdjustPanel/CssPanel/ExportMenu/SavedStrip/Gallery/SeoFooter + `controls/Control.tsx`. Studio's ActionBar "↻ Replay" button bumps a `replayNonce` state used as `<Stage key={replayNonce}>`, remounting Stage to restart every CSS animation on demand (purely visual — doesn't touch `editedCss`/hash/values).
...
- JSON-LD (server-only): home = WebApplication/WebSite/FAQPage; `/effects` = CollectionPage; `/effects/[id]` = SoftwareSourceCode + BreadcrumbList. Crawlable SSR pages: `/effects` (static, grouped) + `/effects/[id]` (196 SSG, full CSS + live preview) are the GEO payload. `next.config.ts` `outputFileTracingIncludes` bundles the OG font.
...
- **Scroll effects** (`caps:['scroll']`): base CSS = the final fully-revealed state; the scrub animation is added inside `@supports (animation-timeline: view())` and only when `ctx.mode !== "thumbnail"` (posters/SSR/PNG/Firefox show the finished state). Anonymous `view()`/`scroll(nearest)` only — named timelines are lint-banned; declare `animation-timeline` AFTER any `animation:` shorthand. Stage wraps scroll effects in a `[data-fx-scroller]` scrubber; standalone export switches to a scrolling body. Both reduce-motion blocks reset `animation-timeline: auto` (duration overrides alone don't stop timeline-driven animations).
- **Color-font / forced-font effects**: `lib/fonts.ts` splits `FONTS` (picker) from `EXTRA_FONTS` (effect-forced only: Nabla, Foldit, Honk, Bungee Inline/Shade, Silkscreen, Sorts Mill Goudy); the app link + export lookup use `ALL_FONTS`. A forced `font:` string must EXACTLY match a FontDef.family or the standalone export ships no font link. Salt `@font-palette-values` names via `prop()` (lint-enforced).
...
- Motion is ON by default and OS-independent in-app: the studio ignores `prefers-reduced-motion`; only an explicit opt-out via the Header MOTION toggle (persisted as `textfx_motion` = `"on"|"off"`) applies `.fx-reduce-motion` to `.app` (re-enabling also bumps `replayNonce` so finished one-shots restart). The `@media (prefers-reduced-motion)` block in `globals.css` guards ONLY the JS-less SSR preview scopes (`fx-card-*`/`fx-effect-*`) — NOT the generic `fx-` prefix, which would crush the studio regardless of the toggle.
- Persistence: favorites in `localStorage` (`textfx_favs_v2`, shape-validated on load, `persistFavorites` returns ok), theme in `localStorage` (`textfx_theme`), motion opt-out in `localStorage` (`textfx_motion`); share state in the URL hash (`#s=`, written only by SHARE and dropped via `stripHash()` on any other state mutation). Client-only init runs in a mount effect; initial render is deterministic (seed 1, theme `dark`, motion on) to avoid hydration mismatch — on mount a saved theme wins, else first visit follows OS `prefers-color-scheme` (then persists); a saved `textfx_motion` applies.
...
- Adding effects: drop a file in `lib/effects/<category>/<id>.ts`, add its import + `EFFECTS` entry to `registry.ts` (append at the end of its category's run), add a `descriptions.ts` entry, add one-shot entrances to `HOVER_REPLAY_IDS` in `tests/engine.test.ts`, bump the hardcoded `N effects` count strings (`lib/site.ts`, `components/SeoFooter.tsx` ×2, `app/effects/page.tsx`, `app/opengraph-image.tsx`), run `pnpm test`. Pre-registration check for a single file: `EFFECT_FILE=lib/effects/<cat>/<id>.ts [EXPECT_ONESHOT=1] pnpm vitest run tests/single-effect.test.ts` (shared assertion bodies live in `tests/effect-contract.ts`).
