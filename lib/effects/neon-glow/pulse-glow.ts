...
const pulseGlow: EffectDefinition = {
  ...
  controls: [
    ...
    { id: "speed", label: "Speed", type: "range", default: 4, min: 1, max: 6, step: 0.1, unit: "s" },
  ],
  rand: (R) => ({
    hue: R.ri(0, 360),
    speed: Number(R.rnd(3.4, 5).toFixed(1)),
  }),
  build: (ctx) => {
    const h = ctx.values.hue as number;
    const speed = ctx.values.speed as number;
    const txt = ctx.theme === "dark" ? hsl(h, 60, 92) : hsl(h, 95, 50);
    const c1 = hsl(h, 60, 60);
    const c2 = hsl((h + 18) % 360, 60, 55);
